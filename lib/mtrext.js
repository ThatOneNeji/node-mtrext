...
const regexPatternMtr = /^\s+?(?<hopID>\d+)[.\|\-\s]+(?<hopAddress>\S+)\s+(?<loss>[A-Z0-9.]+)%?\s+(?<snt>[A-Z0-9.]+)\s+(?<drop>[A-Z0-9.]+)\s+(?<rcv>[A-Z0-9.]+)\s+(?<last>[A-Z0-9.]+)\s+(?<best>[A-Z0-9.]+)\s+(?<avg>[A-Z0-9.]+)\s+(?<wrst>[A-Z0-9.]+)\s+(?<jttr>[A-Z0-9.]+)\s+(?<javg>[A-Z0-9.]+)\s+(?<jmax>[A-Z0-9.]+)\s+(?<jint>[A-Z0-9.]+)$/i;
